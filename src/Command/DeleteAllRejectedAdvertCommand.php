...
#[AsCommand(
    name: 'app:delete-all-rejected-advert',
...
class DeleteAllRejectedAdvertCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($count === 0) {
            $io->success('Aucune annonce ne correspond.');
        ...
        foreach ($result as $advert) {
            $this->advertRepository->remove($advert, true);
            $progress->advance();
        }
        if ($count === 1) {
            $io->success("1 annonce a été supprimé avec succès.");
        } else {
            $io->success("$count annonces ont été supprimés avec succès.");
        }
        return Command::SUCCESS;
    }
}
